...
## Jellium spheres (finite spherical jellium)

The finite spherical jellium model (a.k.a. the spherical "jellium sphere" used to
model closed-shell metal clusters) of
[Sottile and Ballone, Phys. Rev. B 64, 045105 (2001)](https://doi.org/10.1103/PhysRevB.64.045105)
is supported via a custom Hamiltonian in `ferminet/jellium/hamiltonian.py`. `N`
electrons move in the electrostatic potential of a uniform positive background of
charge `+N` confined to a sphere of radius `R_B = N**(1/3) r_s`; the Hamiltonian
adds the external (electron-background) potential `V_ext(r)` and the constant
background self-energy `E_self = (3/5) N**(5/3) / r_s` to the usual bare Coulomb
electron-electron repulsion. Unlike the homogeneous electron gas
(`ferminet/pbc`), this is a *finite, open-boundary* system, so no Ewald summation
is needed; a single charge-zero ghost atom at the origin defines the centre of
the sphere, and the standard isotropic envelope provides the correct decay of the
confined orbitals.

The example config uses the PsiFormer and defaults to the simplest system in the
paper, `N = 2` electrons at the metallic density `r_s = 4`:

```
ferminet --config ferminet/configs/jellium_sphere.py \
    --config.system.n_electrons 2 --config.system.r_s 4
```

Change `n_electrons` to a closed-shell ("magic") size (2, 8, 18, 20, 34, 40, 58,
92, 106) and `r_s` to one of the densities studied (1, 2, 3.25, 4, 5.62). The
config docstring lists the paper's reference VMC/DMC/HF/LDA energies for `N = 2`
for comparison; remember FermiNet reports total energies in Hartree, whereas the
paper reports energies *per electron* in eV
(`E_tot[Ha] = N * E_per_electron[eV] / 27.211386`). The implementation can be
tested with `ferminet/jellium/tests/hamiltonian_test.py`.
